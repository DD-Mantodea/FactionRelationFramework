...
namespace FactionRelationFramework.Patches.FactionPatches
{
    ...
    public static class FactionPatch
    {
        ...
        public static bool PreNotify_GoodwillSituationsChanged(Faction __instance, Faction other, bool canSendHostilityLetter, string reason, GlobalTargetInfo? lookTarget)
        {
            ...
            factionRelation.CheckKindThresholds(__instance, canSendHostilityLetter, reason, lookTarget ?? GlobalTargetInfo.Invalid, out _);
            ...
            if(FactionRelationUtils.TryGetCustomFactionRelationKind(kd1, out var custom))
            {
                ...
                if (legalID != otherID)
                {
                    ...
                    other.Notify_RelationKindChanged(__instance, factionRelation2.kind, canSendHostilityLetter, rea, lookTarget ?? GlobalTargetInfo.Invalid, out _);
                }
            }
            else if (kd1 != kd2 && !FactionRelationUtils.CustomFactionRelationKindExist(kd2))
            {
                ...
                other.Notify_RelationKindChanged(__instance, kd2, canSendHostilityLetter, reason, lookTarget ?? GlobalTargetInfo.Invalid, out _);
            }
